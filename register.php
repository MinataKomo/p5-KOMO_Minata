...
<?php
require('config.php');
if (isset($_REQUEST['nom'], $_REQUEST['email'], $_REQUEST['passe'])){
  ...
  //requéte SQL + mot de passe crypté
    $query = "INSERT into `connecte` (nom, email, passe)
              VALUES ('$nom', '$email', '".hash('sha256', $passe)."')";
...
<form class="box" action="" method="post">
  ...
  <input type="text" class="box-input" name="nom" placeholder="Nom d'utilisateur" required />
    ...
    <input type="password" class="box-input" name="passe" placeholder="Mot de passe" required />
